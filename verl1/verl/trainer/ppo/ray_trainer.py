            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        # Check if reward_fn has compute_for_rollout method (for GRPO, SRPO, etc.)
                        # Note: compute_for_rollout will be called after old_log_prob is computed
                        # to ensure entropys are available
                        use_compute_for_rollout = hasattr(self.reward_fn, 'compute_for_rollout')
                        
                        # Initialize variables for reward computation
                        reward_tensor = None
                        reward_extra_infos_dict = {}
                        rewards_tensor = None
                        future_reward = None
                        
                        if not use_compute_for_rollout:
                            # Standard reward computation path
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            # If using compute_for_rollout, compute rewards and advantages here
                            # after entropys are available
                            if use_compute_for_rollout:
                                # Extract ground truth from non_tensor_batch
                                ground_truth = []
                                for i in range(len(batch)):
