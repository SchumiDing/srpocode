...
@register("srpo3")
class SRPO3RewardManager(AbstractRewardManager):
    """SRPO (Segment-based Reward Policy Optimization) Reward Manager.
    
    SRPO computes both segment-level and sequence-level rewards, then combines them
    with normalization to compute advantages.
    """
    ...
    def __init__(
        ...
        conversation_parser: str = "deepseek",  # "qwen", "llama3", "deepseek", or "auto"
        **kwargs: Any,
    ):
    ...
    def compute_for_rollout(
        ...
        rollout_raw_reward = [info['seq_reward'] + mean_seqReward[i] for i, info in enumerate(per_sample_info)]
        ...
        print(f"{i}th Mean Seq Reward: ", mean_seqReward[i], flush=True)
