...
# Algorithm selection: grpo | srpo | srpo2 | srpo3 | srpo4 | rflux
ALGORITHM=${ALGORITHM:-grpo}
# Map to matching advantage estimator (reward managers no longer compute advantages)
case "$ALGORITHM" in
...
# Reward manager name follows algorithm
reward_manager=${ALGORITHM}
...
# Cluster topo: set NNODES for multi-node (e.g., NNODES=2 with 8 H200 per node)
NNODES=${NNODES:-1}
# Paths
...
python3 -m verl.trainer.main_ppo \
    ...
    reward_manager.name=${reward_manager} \
    +reward_model.reward_kwargs.overlong_buffer_cfg.enable=${enable_overlong_buffer} \
    +reward_model.reward_kwargs.overlong_buffer_cfg.len=${overlong_buffer_len} \
    +reward_model.reward_kwargs.overlong_buffer_cfg.penalty_factor=${overlong_penalty_factor} \
    +reward_model.reward_kwargs.overlong_buffer_cfg.log=False \
    +reward_model.reward_kwargs.max_resp_len=${max_response_length} \
    +reward_model.reward_kwargs.conversation_parser="deepseek" \
