...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - beta/mean, max, min, std: Statistics about beta values (if beta field exists in batch)
    """
    ...
    # beta field metrics
    if "beta" in batch.batch:
        beta_tensor = batch.batch["beta"]
        if beta_tensor.dim() > 1:
            # If beta is token-level, compute sequence-level statistics
            sequence_beta = beta_tensor.sum(-1)
        else:
            # If beta is already sequence-level
            sequence_beta = beta_tensor
            
        # Compute statistics for non-aborted samples only
        non_aborted_beta = sequence_beta[non_aborted_mask] if non_aborted_mask.any() else sequence_beta
        
        if non_aborted_beta.numel() > 0:
            metrics["outcomerewards/mean"] = torch.mean(non_aborted_beta).detach().item()
            metrics["outcomerewards/max"] = torch.max(non_aborted_beta).detach().item()
            metrics["outcomerewards/min"] = torch.min(non_aborted_beta).detach().item()
            metrics["outcomerewards/std"] = torch.std(non_aborted_beta).detach().item()
        else:
            # Fallback to all samples if no non-aborted samples
            metrics["outcomerewards/mean"] = torch.mean(sequence_beta).detach().item()
            metrics["outcomerewards/max"] = torch.max(sequence_beta).detach().item()
            metrics["outcomerewards/min"] = torch.min(sequence_beta).detach().item()
            metrics["outcomerewards/std"] = torch.std(sequence_beta).detach().item()
