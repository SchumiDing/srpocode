...
class TensorDict:
    def __init__(self, data=None, batch_size: int = None):
        """
        data: mapping of key -> tensor-like (torch.Tensor, numpy, list, etc.)
        batch_size: optional hint (not strictly required). If provided we won't validate strictly.
        """
        self._data = {}
        if data is None:
            data = {}
        # If input is another TensorDict, copy underlying mapping shallowly
        if isinstance(data, TensorDict):
            for k, v in data.items():
                self._data[k] = v
        # If mapping (e.g., dataloader returned dict)
            ...
            for k, v in data.items():
                ...
                if not isinstance(v, torch.Tensor):
                    try:
                        ...
                        # fallback: keep as-is (some objects may be fine to keep)
                        pass
                self._data[k] = v
        else:
            raise TypeError("TensorDict expects a mapping or TensorDict as input")

        # store optional batch_size for later use / validation (not strictly required)
        self.batch_size = batch_size
        if self.batch_size is None:
            # try to infer from first tensor value
    ...
    def __repr__(self):
        keys = ", ".join(list(self._data.keys()))
        return f"TensorDict(keys=[{keys}], batch_size={self.batch_size})"

    def to(self, device):
        """递归地将所有 tensor 移到指定 device"""
        
        def move(v):
            return torch.tensor(v.cpu().numpy(), device=device)

        for k, v in self._data.items():
            self._data[k] = move(v)
        return self
    def clone(self):
        """Return a shallow clone (tensors are not deep-copied)."""
    ...
    def split(self, micro_batch_size: int):
        """
        Split along dimension 0 into a list of TensorDicts each of size micro_batch_size (last one may be smaller).
        This mirrors typical micro-batching behavior used in training_step.
        """
        ...
        if total is None:
            ...
            for v in self._data.values():
                if isinstance(v, torch.Tensor) and v.ndim >= 1:
                    ...
                    break
            if total is None:
                raise ValueError("Cannot determine batch size for split; provide batch_size on construction")

        out = []
        for start in range(0, int(total), int(micro_batch_size)):
            end = min(int(total), start + int(micro_batch_size))
            sub = {}
            for k, v in self._data.items():
                if isinstance(v, torch.Tensor):
                    # guard in case tensor is smaller than expected
                    if v.ndim == 0:
                        # scalar tensor, keep as-is
                        sub[k] = v
                    else:
                        # safe slice (if shape[0] shorter than end, take available)
                        sub[k] = v[start:end].contiguous()
                else:
                    # non-tensor: if it's indexable, slice; else keep as-is
                    try:
                        sub[k] = v[start:end]
                    except Exception:
                        sub[k] = v
            td = TensorDict(sub, batch_size=(end - start))
            out.append(td)
        return out

    # allow iteration like a mapping
    def __iter__(self):
    ...
    # convenience: to convert back to plain dict of tensors
    def as_dict(self):
...
class FSDPSFTTrainer:
    ...
    def _build_dataloader(self, train_dataset, val_dataset):
        ...
        with open("temp.txt", "w") as f:
            f.write("train_sampler: " + str(len(self.train_sampler)))
            f.write("\n")
            f.write("train_dataset: " + str(len(self.train_dataset)))
        ...
        with open("temp.txt", "a") as f:
            f.write("train_dataloader: " + str(len(self.train_dataloader)))
    ...
    def fit(self):
        ...
        if rank == 0:
            tracking = Tracking(
                ...
                config=OmegaConf.to_container(OmegaConf.create(self.config), resolve=True),
            )
...
def run_sft(config):
    ...
    # if chat_template not in tokenizer:
